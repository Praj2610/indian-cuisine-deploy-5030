    ...
    if select == charts[0]:
        ...
        if select == flavorChart[0]:
            ...
            fig.update_layout(
                ...
                width=800,
                height=800,
                xaxis_title="Flavoured Profile Index",    # Label for the x-axis
...
#####################################################################################################################################
    ...
    def recommend_tfidf_cosine(text):
        if not text:  # Check if the input text is empty or None
            raise ValueError("Please provide appropriate ingredients")
        ...
        if accuracy > 0.00:
            for index, recipe in enumerate(final_list, start=1):
                if data['EnglishRecepie'].isin([recipe]).any():
                    ...
                    #st.write(index,end="")
                    #st.markdown(f" <span style='color:#fa8072'>{recipe}(https://www.google.com)</span>", unsafe_allow_html=True)
